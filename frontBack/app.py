...
async def processAudio(file : UploadFile = File(...)):
    ...
    mindMapJSON = await createMindMap(abstract)
    print(mindMapJSON)

    try:
        mindMapData = json.loads(mindMapJSON)
    except json.JSONDecodeError:
        raise HTTPException(status_code=500, detail="LLM returned an invalid JSON of the mind map.")
    return{
        "abstract": abstract,
        "mindMap": mindMapData
    }
